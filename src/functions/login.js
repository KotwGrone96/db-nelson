    ...
    connection.query(
      ...
      (err, result) => {
        if (err) {
          res.json({
            err,
            msg: "Ha ocurrido un error en la consulta a la base de datos",
            ok: false,
          });
          return;
        }
        ...
        const { idStatus, id, idPermiso } = result[0];
              ...
              connection.query(
                ...
                (errUpdate, resultUpdate) => {
                  if (errUpdate) {
                    res.json({
                      errUpdate,
                      msg: "Ha ocurrido un error en la actualización de la base de datos",
                      ok: false,
                    });
                    return;
                  }
